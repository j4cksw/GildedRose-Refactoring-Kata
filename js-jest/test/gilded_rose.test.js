const {Shop, Item, AgedBrie, BackstagePassesToATAFKAL80ETCConcert, Sulfuras, ConjuredManaCake} = require("../src/gilded_rose");
...
describe("Gilded Rose", function() {
  ...
  it("Backstage passes...", () => {
    const gildedRose = new Shop([new Item(BackstagePassesToATAFKAL80ETCConcert, 0, 0)]);
    expect(gildedRose.updateQuality()).toEqual([new Item(BackstagePassesToATAFKAL80ETCConcert, -1, 0)])
    expect(gildedRose.updateQuality()).toEqual([new Item(BackstagePassesToATAFKAL80ETCConcert, -2, 0)])
    
    for(let i=0;i<100;i++) {
      gildedRose.updateQuality();
    }
    expect(gildedRose.updateQuality()).toEqual([new Item(BackstagePassesToATAFKAL80ETCConcert, -103, 0)]);
  });

  it("Backstage passes with sellIn equal to 11 and quality lower than 50 will increase quality by 1", () => {
    const gildedRose = new Shop([new Item(BackstagePassesToATAFKAL80ETCConcert, 11, 49)]);
    expect(gildedRose.updateQuality()).toEqual([new Item(BackstagePassesToATAFKAL80ETCConcert, 10, 50)])
  });

  it("Backstage passes with sellIn lower than 11 and quality lower than 50 will increase quality by 1", () => {
    const gildedRose = new Shop([new Item(BackstagePassesToATAFKAL80ETCConcert, 10, 49)]);
    expect(gildedRose.updateQuality()).toEqual([new Item(BackstagePassesToATAFKAL80ETCConcert, 9, 50)])
  });

  it("Backstage passes with sellIn equal to 6 and quality lower than 50 will increase quality by 1", () => {
    const gildedRose = new Shop([new Item(BackstagePassesToATAFKAL80ETCConcert, 6, 49)]);
    expect(gildedRose.updateQuality()).toEqual([new Item(BackstagePassesToATAFKAL80ETCConcert, 5, 50)])
  });

  it("Backstage passes with sellIn lower than 6 and quality lower than 50 will increase quality by 1", () => {
    const gildedRose = new Shop([new Item(BackstagePassesToATAFKAL80ETCConcert, 5, 49)]);
    expect(gildedRose.updateQuality()).toEqual([new Item(BackstagePassesToATAFKAL80ETCConcert, 4, 50)])
  });

  it("Backstage passes with sellIn lower than 6 and quality lower than 50 will increase quality by 3", () => {
    const gildedRose = new Shop([new Item(BackstagePassesToATAFKAL80ETCConcert, 5, 10)]);
    expect(gildedRose.updateQuality()).toEqual([new Item(BackstagePassesToATAFKAL80ETCConcert, 4, 13)])
  });
  ...
  it("conjured item will decrease quality by half", () => {
    ...
    expect(gildedRose.updateQuality()).toEqual([new Item(ConjuredManaCake, 0, 5)]);
  });


  it("conjured item's quality can not lower than zero", () => {
    const gildedRose = new Shop([new Item(ConjuredManaCake, 1, 0)]);
    expect(gildedRose.updateQuality()).toEqual([new Item(ConjuredManaCake, 0, 0)]);
  });
});
